...
class ClipCocoDataset(Dataset):
    ...
    def pad_tokens(self, item: int):
        ...
        if padding > 0:
            tokens = torch.cat(
                (tokens, torch.zeros(padding, dtype=torch.int64)))
        elif padding < 0:
    ...
    def __init__(self, data_path: str,  prefix_length: int, gpt2_type: str = "gpt2",
        ...
        for caption in self.captions[:]:
            try:
                self.captions_tokens.append(torch.tensor(
                    self.clip_tokenizer(caption)[0], dtype=torch.int64))
            except:
                continue
...
def train_decoder(dataset: ClipCocoDataset, args,
    ...
    train_dataloader = DataLoader(
        dataset, sampler=sampler, batch_size=batch_size, drop_last=True)
    ...
    for epoch in range(epochs):
        ...
        if args.is_master:
            ...
            progress = tqdm(total=int(len(train_dataloader)/10),
                            desc=output_prefix)

        dist.barrier()
        for idx, (clip_tokens, clip_tokens_77) in enumerate(train_dataloader):
            clip_tokens, clip_tokens_77 = clip_tokens.to(
                device), clip_tokens_77.to(device)
            ...
            ac = ((logits.argmax(1) == clip_tokens) *
                  (clip_tokens > 0)).sum()/(clip_tokens > 0).sum()
            optimizer.zero_grad()
            loss_all = loss_token
            loss_all.backward()
            optimizer.step()
            ...
            if args.is_master:
                ...
                if (idx+1) % 10 == 0:
                    progress.set_postfix(
                        {"loss_token": loss_token_save/10.0, "acc_token": ac_save/10.0})
                    progress.update()
                    loss_token_save, ac_save = 0, 0
                else:
...
def main():
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '--data', default='./data/coco/doubleMTA_lr1e5_ignore0.pkl')
    ...
    parser.add_argument('--dataset', default='coco',
                        help='coco or cc3m or bookcorpus')
    ...
    parser.add_argument('--only_prefix', dest='only_prefix',
                        action='store_true')
    parser.add_argument('--mapping_type', type=str,
                        default='mlp', help='mlp/transformer')
    parser.add_argument('--num_layers', type=int, default=8)
    parser.add_argument('--is_rn', dest='is_rn', action='store_true')
    parser.add_argument('--normalize_prefix',
                        dest='normalize_prefix', action='store_true')
    parser.add_argument('--local_rank', type=int, default=-1,
                        metavar='N', help='Local process rank.')
    args = parser.parse_args()
    prefix_length = args.prefix_length
    dataset = ClipCocoDataset('data/'+args.dataset+'_train.json',
                              prefix_length, normalize_prefix=args.normalize_prefix)

    train_decoder(dataset, args, output_dir=args.out_dir,
