...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        """
        x = self.sublayer_connection_1(x, self.feed_forward)
        """
        return x
...
class EncoderLayerNoAttention(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, multiple_of: int, dropout: float, first_layer: bool):
        ...
        self.feed_forward = SwiGLU(d_model=d_model, d_ff=d_ff, multiple_of=multiple_of, dropout=dropout)
        if first_layer: self.sublayer_connection = SkipConnectionAfterLN(size=d_model, dropout=dropout)
        else:           self.sublayer_connection = SublayerConnection(size=d_model, dropout=dropout)
...
class R2LDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, multiple_of: int, num_heads: int, sublayer_num: int, dropout: float, use_rope: bool, first_layer: bool):
        ...
        """
        if first_layer: self.enc_sublayer_connection_1 = SkipConnectionAfterLN(size=d_model, dropout=dropout)
        else:           self.enc_sublayer_connection_1 = SublayerConnection(size=d_model, dropout=dropout)
        self.enc_sublayer_connection_2 = SublayerConnection(size=d_model, dropout=dropout)
        self.enc_attn = MultiHeadAttention(num_heads=num_heads, d_model=d_model, dropout=dropout, use_rope=use_rope)
        self.enc_feed_forward = SwiGLU(d_model=d_model, d_ff=d_ff, multiple_of=multiple_of, dropout=dropout)
        """
    ...
    def forward(self, x, memory, src_mask, trg_mask, r2l_memory=None, r2l_trg_mask=None):
        ...
        """
        memory = self.enc_sublayer_connection_1(memory, lambda memory: self.enc_attn(memory, memory, memory, src_mask))
        memory = self.enc_sublayer_connection_2(memory, self.enc_feed_forward)
        """
...
class Encoder(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, multiple_of: int, num_heads: int, num_layers: int, dropout: float, use_rope: bool):
        ...
        assert self.num_layers > 0, f"[Encoder.__init__] self.num_layers={self.num_layers}"
    ...
    def forward(self, x, src_mask):
        ...
        assert self.num_layers > 0, f"[Encoder.forward] self.num_layers={self.num_layers}"
...
def pad_mask(src, r2l_trg, trg, pad_idx):
    if isinstance(src, tuple):
        ...
        if len(src) == 3:
            mask_1 = (src[0][:, :, 0] != pad_idx)
            assert mask_1.ndim == 2, f"[pad_mask] mask_1.ndim={mask_1.ndim}"
            #assert mask_1.shape[1] == 10, f"[pad_mask] mask_1.shape={mask_1.shape}"

            mask_2 = (src[1][:, :, 0] != pad_idx)
            assert mask_2.ndim == 2, f"[pad_mask] mask_2.ndim={mask_2.ndim}"
            #assert mask_2.shape[1] == 10, f"[pad_mask] mask_2.shape={mask_2.shape}"

            mask_3 = (src[2][:, :, 0] != pad_idx)
            assert mask_3.ndim == 2, f"[pad_mask] mask_3.ndim={mask_3.ndim}"
            #assert mask_3.shape[1] == 10, f"[pad_mask] mask_3.shape={mask_3.shape}"

            _stacked = torch.stack([mask_1, mask_2, mask_3], dim=2)
            out_mask = _stacked.reshape(mask_1.size(0), -1)
            ...
            assert torch.equal(out_mask[:,0::3], mask_1[:,:]) == True
            assert torch.equal(out_mask[:,1::3], mask_2[:,:]) == True
            assert torch.equal(out_mask[:,2::3], mask_3[:,:]) == True

            #enc_src_mask = out_mask.unsqueeze(1)'
            enc_src_mask = (mask_1.unsqueeze(1), mask_2.unsqueeze(1), mask_3.unsqueeze(1))
            #dec_src_mask = out_mask.unsqueeze(1)'
            dec_src_mask = (mask_1 | mask_2 | mask_3).unsqueeze(1)
    ...
    if trg is not None:
        if isinstance(src_mask, tuple):
            trg_mask = (trg != pad_idx).unsqueeze(1) & subsequent_mask(trg.size(1)).type_as(mask_1.data)
            r2l_pad_mask = (r2l_trg != pad_idx).unsqueeze(1).type_as(mask_1.data)
            r2l_trg_mask = r2l_pad_mask & subsequent_mask(r2l_trg.size(1)).type_as(mask_1.data)
...
class ABDTransformer(nn.Module):
    ...
    def encode(self, src, src_mask, feature_mode_two=False):
        ...
        if feature_mode_two:
            ...
            """
            x1 = x1 * src_mask[0].squeeze(1).unsqueeze(-1).to(x1.dtype)
            x2 = x2 * src_mask[1].squeeze(1).unsqueeze(-1).to(x2.dtype)
            x3 = x3 * src_mask[2].squeeze(1).unsqueeze(-1).to(x3.dtype)
            """
            return x1 + x2 + x3
