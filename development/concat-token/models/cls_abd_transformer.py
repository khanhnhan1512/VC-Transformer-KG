...
def pad_mask(src, r2l_trg, trg, pad_idx):
    if isinstance(src, tuple):
        ...
        if len(src) == 3:
            src_image_mask = (src[0][:, :, 0] != pad_idx).unsqueeze(1)
            _pad_src_image_mask = src_image_mask[:,:,:1]
            src_image_mask = torch.cat((_pad_src_image_mask, src_image_mask), dim=2)
            #assert src_image_mask.shape[1:] == (1, 11), f"src_image_mask.shape = {src_image_mask.shape}"

            src_motion_mask = (src[1][:, :, 0] != pad_idx).unsqueeze(1)
            _pad_src_motion_mask = src_motion_mask[:,:,:1]
            src_motion_mask = torch.cat((_pad_src_motion_mask, src_motion_mask), dim=2)
            #assert src_motion_mask.shape[1:] == (1, 11), f" src_motion_mask.shape = { src_motion_mask.shape}"

            src_object_mask = (src[2][:, :, 0] != pad_idx).unsqueeze(1)
            _pad_src_object_mask = src_object_mask[:,:,:1]
            src_object_mask = torch.cat((_pad_src_object_mask, src_object_mask), dim=2)
            #assert src_object_mask.shape[1:] == (1, 11), f" src_object_mask.shape = { src_object_mask.shape}"

            enc_src_mask = (src_image_mask, src_motion_mask, src_object_mask)
            dec_src_mask = src_image_mask & src_motion_mask & src_object_mask
            #assert dec_src_mask.shape[1:] == (1, 11), f"dec_src_mask.shape = {dec_src_mask.shape}"
...
class ABDTransformer(nn.Module):
    ...
    def encode(self, src, src_mask, feature_mode_two=False):
        ...
        if feature_mode_two:
            ...
            return x1 + x2 + x3
            #return (x1[:,0,:] + x2[:,0,:] + x3[:,0,:]).unsqueeze(1)
            #return (x1 + x2 + x3).mean(dim=1, keepdim=True)
