...
% TODO: New Feature Extraction
% \textbf{Feature Extraction}. Following the process in 
% Section~\ref{subsec:subsec_42_extraction}, all multimodal features are pre-extracted offline and stored in HDF5 format. Appearance features are extracted from I-frames using BLIP-2 OPT-2.7B ($d_A\,\texttt{=}\,1{,}408$), semantic features from BLIP-2 captions using SRoBERTa-large ($d_S\,\texttt{=}\,1{,}024$), and motion features from 16 sampled frames per GOP using MViTv2-small ($d_M\,\texttt{=}\,768$).
...
% TODO: (OPTIONAL) Short-version of design's contributions
Across benchmarks, several consistent patterns reinforce the proposed design choices. BiDecT substantially outperforms BTKG, the closest prior bidirectional decoding approach (+33.5 CIDEr on MSVD, +9.4 on MSR-VTT), despite BTKG relying on additional encoders and knowledge graph augmentation. It also consistently surpasses CoCap, which shares the compressed video representation, confirming that GOP-based segmentation requires complementary multimodal features to be effective. Finally, BiDecT outperforms every knowledge graph-augmented method, suggesting that VLM-derived semantic representations offer a structurally simpler yet effective alternative to explicit knowledge graph pipelines.
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.90\textwidth, height=8.5in, keepaspectratio]{figures/fig06_exp_hyperparams.png}
\vspace{-6pt}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.90\columnwidth]{figures/fig07_exp_decoding_comparison.png}
\vspace{-4pt}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.90\textwidth, height=8.5in, keepaspectratio]{figures/fig08_exp_prc_analysis.png}
\vspace{-6pt}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.94\textwidth, height=8.5in, keepaspectratio]{figures/fig10_case_study.png}
