...
The extracted appearance, semantic, and motion features reside in heterogeneous feature spaces of different dimensionalities. Modality-specific linear projections therefore map them into a common $d_{model}$-dimensional representation space:
\begin{align}
F_A^\prime&={\mathrm{Linear}}_A(F_A),\\
F_S^\prime&={\mathrm{Linear}}_S(F_S),\\
F_M^\prime&={\mathrm{Linear}}_M(F_M),
\end{align}
where $F_A^\prime,F_S^\prime,F_M^\prime\in\mathbb{R}^{G\times d_{model}}$. Using separate projection matrices ensures that each modality undergoes a tailored transformation that accounts for its distinct representational characteristics.

After projection, the three feature sequences cannot be differentiated by the decoder unless their origins are explicitly encoded. Each projected feature is therefore augmented with a learnable type embedding \mbox{$\left(\mathrm{TE}\in\mathbb{R}^{d_{model}}\right)$} indicating its source modality, and a shared sinusoidal positional embedding \mbox{$\left(\mathrm{PE}\in\mathbb{R}^{G\times d_{model}}\right)$} is added to encode the temporal order of GOPs across all modalities. Modality-specific normalization layers are then applied to stabilize feature magnitudes:
\begin{align}
...
\begin{equation}
E=\mathrm{Concat}(E_A,E_S,E_M)\in\mathbb{R}^{3G\times d_{model}}.
\end{equation}
...
The causal mask in masked self-attention ensures that position $t$ attends only to positions $(1,\ldots,t\texttt{-}1)$. The BD comprises $N_{BD}$ stacked decoder layers. The hidden state from the final layer is normalized to obtain the per-step backward hidden representation:
\begin{equation}
\MyLVec[1]{h_{t}} = \mathrm{Norm}\Big( \MyLVec[1]{Z_{t;N_{BD}}} \Big).
\end{equation}
...
Beyond generating the reverse caption, the BD produces a sequence of hidden states encoding a right-to-left semantic representation, collectively forming the global backward context \mbox{$\MyLVec[1]{H} = [\MyLVec[1]{h_{1}},\ldots,\MyLVec[1]{h_{T^\prime}}] \in\mathbb{R}^{T^\prime \times d_{model}}$}. This contextual representation is subsequently passed to the Forward Decoder as an auxiliary conditioning signal. Crucially, $\MyLVec[1]{H}$ is not a static encoded representation, but a dynamically generated contextual sequence derived from reverse caption generation. Its quality therefore depends on how effectively the BD learns semantically coherent reverse captions during joint optimization with the Forward Decoder.
...
The FD comprises $N_{FD}$ stacked decoder layers. The hidden state from the final layer is normalized and then used to compute the probability distribution over the vocabulary at decoding step $t$:
\begin{equation}
\MyRVec[1]{h_{t}} = \mathrm{Norm}\Big( \MyRVec[1]{Z_{t;N_{FD}}} \Big),
\end{equation}
\begin{equation}
...
\begin{align}
\mathcal{L}_{BD} &= - \sum_{t=1}^{L^\prime} \mathrm{log}\; P\Big(\MyLVec[1]{y_{t}} \;|\, \MyLVec[1]{Y_{<t}}, \MyLVec[1]{E}; \, \theta_{BD} \Big),
\\
\mathcal{L}_{FD} &= - \sum_{t=1}^{L} \mathrm{log}\; P\Big(\MyRVec[1]{y_{t}} \;|\, \MyRVec[1]{Y_{<t}},\MyRVec[1]{E},\MyLVec[1]{H}; \, \theta_{BD},\theta_{FD} \Big),
\end{align}
where $\theta_{BD}$ and $\theta_{FD}$ denote the trainable parameters of the backward and forward decoders, respectively. Importantly, $\mathcal{L}_{FD}$ explicitly depends on $\theta_{BD}$ because the FD conditions on the global backward context $\MyLVec[1]{H}$ generated by the BD. Gradients from $\mathcal{L}_{FD}$ therefore propagate through the BD, allowing $\MyLVec[1]{H}$ to be jointly optimized for both reverse and forward caption generation.

The overall training objective combines both terms:
\begin{equation}
\mathcal{L}=(1\texttt{-}\lambda)\,\mathcal{L}_{BD}+\lambda\,\mathcal{L}_{FD},
\end{equation}
...
\begin{equation}
\mathcal{O}\left(G^2\cdot d_{model}+G\cdot d_{model}^2\right).
\end{equation}
...
\label{tab:tab01_complexity}
\centering
\begin{tabular}{l c c}
\hline\hline
\textbf{Component} & \textbf{Encoder-based} & \textbf{Encoder-free (BiDecT)} \\
\hline
Encoding stage & $\mathcal{O}(G^2 \cdot d_{\mathrm{model}})$ & --- \\
Decoding stage & \multicolumn{2}{c}{$\mathcal{O}((L+L^\prime) \cdot G \cdot d_{\mathrm{model}})$} \\
...
% Table 1 
% Computational complexity comparison between standard encoder-decoder architectures and BiDecT per decoder layer, as a function of the number of GOPs $G$.
% Component	Standard Enc-Dec	BiDecT
% Encoding stage	\mathcal{O}(G^2\cdot d_{\mathrm{model}})	—
% Cross-attention to multimodal	\mathcal{O}(\left(L+L^\prime\right)\cdot G\cdot d_{\mathrm{model}})	\mathcal{O}(\left(L+L^\prime\right)\cdot G\cdot d_{\mathrm{model}})
% Self-attention (captions)	\mathcal{O}(\left(L^2+L^{\prime2}\right)\cdot d_{\mathrm{model}})	\mathcal{O}(\left(L^2+L^{\prime2}\right)\cdot d_{\mathrm{model}})
% Dominant scaling in G	Quadratic \mathcal{O}(G^2)	Linear O(G)
