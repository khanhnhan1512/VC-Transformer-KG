...
- **Link:** Received: 31 July 2021 / Accepted: 21 July 2022 / Published online: 12 August 2022

- **Published in:** Machine Vision and Applications (2022) 33:77 https://doi.org/10.1007/s00138-022-01329-3
...
```
Video captioning recently has attracted much attention, due to its wide range of applications such as video retrieval [35,37], visual question answering (VQA) [4,11,23], and assisting visually impaired people [28]. Video captioning is such a task which tries to generate some semantic descriptions of some given videos. Thus, video captioning is a multi-modal problem involving at least video and natural language processing.

Traditional video captioning methods are generally template based, that is, firstly identifying the semantics words of objects, activities, and scenes in the video and then matching them via a pre-defined sentence template [9,15,21].  However, the quality of the sentences generated by these methods may depend heavily on the pre-defined templates, usually yielded via a hand-crafted way.  Motivated by the great success of deep learning in video and natural language processing, the current predominated methods for video captioning are based on the deep encoderdecoder framework, where an encoder is utilized to extract the content of videos and a decoder is exploited to generate the semantic descriptions [26,27]. The attention mechanism was later introduced in [34] to improve the performance of this encoder–decoder-based framework. In addition, in the field of image captioning, [31,32] propose solutions to the problem of out-of-vocabulary.  Inspired by the superior performance of Transformer network [24] over CNNs on extracting the sequential representation, Transformer was recently adopted to video captioning in [5,17,41]. Specifically, [41] firstly introduced the Transformer network for video captioning with a significant improvement on the performance. Built upon such Transformer-based video captioning framework, [5] later introduced two fusion blocks to fuse different modal information and thus gain some further improvement on the performance.  However, the existing video captioning methods based on the deep encoder–decoder framework may not fully exploit the context of a video, i.e., only exploiting the left-to-right context in the training but leaving the right-to-left context unexploited. As a result, the generated video caption may miss some important semantic description, as shown in Fig. 1.  Since the model usually predicts backwards word by word, after predicting “talking”, a model that only uses forward decoder may tend to generate an end marker such as “< EOS>”, which may lose a lot of details. However, the model using the bidirectional decoder is more likely to predict a word that is not “< EOS> ” , because the backward decoder get backward information in advance, which makes the final video captions more accurate.  In this paper, we propose a novel video captioning method based on Transformer, by exploiting bidirectional context. In order to exploit the bidirectional context, we introduce a bidirectional (forward-backward) decoder instead of a unidirectional decoder (called forward decoder in this paper) used in the literature. Specifically, in the bridge of encoder and forward decoder, we introduce a backward decoder to effectively exploit the right-to-left context. By the use of such bidirectional decoder, the context information is more fully exploited, resulting in better video captions. The major contributions of this paper can be summarized as follows: (a) We propose an effective Transformer-based video captioning method called bidirectional Transformer (dubbed BiTransformer), by equipping with a bidirectional decoder to exploit both the left-to-right and right-toleft styles of context. As shown in Fig. 1, the suggested  model can significantly improve the quality of the generated video captions. (b) The effectiveness of the proposed model is demonstrated on two benchmark datasets (i.e., MSVD and MSR-VTT). The numerical results show that the performance of the suggested model is competitive to the state-of-the-art methods in the perspective of several important evaluation metrics including CIDEr, BLEU@4, ROUGE and METEOR. Particularly, in terms of CIDEr, the proposed model gains improvements of 1.2% over the state-of-the-art methods in both datasets. Moreover, the generated captions by our model generally have higher quality in the perspective of semantic understanding.
```

- **Related work:**

```
In this section, we provide some related work to this paper.  2.1 Video captioning  Video captioning is to automatically generate descriptions of video content. In recent years, with the development of deep learning, many video captioning methods based on deep encoder–decoder framework have been proposed in the literature [1,7,20,26,29,30,34,36,39]. In [41], the authors proposed an end-to-end transformer model to solve the problem of simultaneous detection and description of events in dense video captioning tasks. Based on [5,12,41] proposed a sparse boundary aware transformer to reduce redundant visual features of different time steps in video representation [17] recently introduced a memory module based on Transformer to generate a highly summarized memory state from the video segments and the sentence history so as to improve prediction of the description. However, these existing methods [5,12,17,40,41] do not fully exploit the context information of the video, that is, only exploiting the left-to-right context information in the training, but leaving the right-to-left context information unexploited. This paper introduces a bidirectional decoder to fully exploit the context information, which is novel and significantly different to the literature.  2.2 Bidirectional decoding  The idea of bidirectional decoding has been recently utilized in the fields of machine translation [38,42] and speech recognition [6] in the purpose of fully exploiting the context information. In [38], a recurrent neural network (RNN)based bidirectional decoder was introduced to deeply capture the bidirectional context information in machine translation, while in [6], the authors introduced certain bidirectional beam search method to fully exploit the bidirectional context information in speech recognition. Distinguished from these papers, we use a Transformer-based bidirectional decoder to deeply mine the context information, due to the superior performance of Transformer in the tasks of natural language processing [24].
```
