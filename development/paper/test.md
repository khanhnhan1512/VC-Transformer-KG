...
$$F_A = [a^{(1)}, \dots, a^{(G)}], \text{ where } a^{(g)} \in \mathbb{R}^{d_A} \text{ and } F_A \in \mathbb{R}^{G \times d_A}.$$
2. Semantic feature:
$$F_S = [s^{(1)}, \dots, s^{(G)}], \text{ where } s^{(g)} \in \mathbb{R}^{d_S} \text{ and } F_S \in \mathbb{R}^{G \times d_S}.$$
3. Motion feature:
$$F_M = [m^{(1)}, \dots, m^{(G)}], \text{ where } m^{(g)} \in \mathbb{R}^{d_M} \text{ and } F_M \in \mathbb{R}^{G \times d_M}.$$

Here, $d_A$, $d_S$, and $d_M$ are the hidden dimensions of the respective pre-trained models. These three features provide a rich multimodal foundation for the next multimodal feature embedding stage.
