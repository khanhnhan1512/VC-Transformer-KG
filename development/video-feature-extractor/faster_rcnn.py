...
# Use same normalization as ImageNet pretrained models
...
def resize_keep_aspect_and_tensor(img: Image.Image,
    ...
    resized = F.resize(img, (new_h, new_w))  # (H, W)
    tensor = F.to_tensor(resized)            # [C, H, W], values [0,1]
...
def pad_batch_tensors(tensors: List[torch.Tensor]) -> Tuple[torch.Tensor, List[Tuple[int, int]]]:
    """
...
def extract_embeddings_from_keyframes(keyframes: List[Image.Image],
    ...
    for i, img in enumerate(keyframes):
        t, resized_hw, (scale_h, scale_w) = resize_keep_aspect_and_tensor(
            img, short_side=short_side, max_size=max_size)
        prepared.append({
    ...
    for start in range(0, N, batch_size):
        ...
        batch_tensor, shapes = pad_batch_tensors(images_list)  # [B,C,Hmax,Wmax] on device
        ...
        if pooled.shape[0] == 0:
            ...
            for idx_in_batch, item in enumerate(batch_items):
                results.append({
                    ...
                    'objects': [],
                    # fallback dim
                    'image_embedding': np.zeros(1024 + 4, dtype=np.float32)
                })
            continue
        ...
        for i_in_batch, (item, out, boxes_resized, scores_resized, labels_resized, emb_t) in enumerate(
            ...
            objs = []
            if emb_t.shape[0] == 0:
                image_embedding = np.zeros(box_features.shape[1] + 4, dtype=np.float32)
            else:
...
def run() -> None:
    ...
    # open HDF5
    print("Extracting features and saving to MSVD_FasterRCNN.hdf5...")
    with h5py.File("./MSVD_FasterRCNN.hdf5", "w") as hf:
    ...
    print("Verifying saved HDF5 file...")
    with h5py.File("./MSVD_FasterRCNN.hdf5", "r") as f:
        # liệt kê các key
        keys = list(f.keys())
        print("#keys:", len(keys))
